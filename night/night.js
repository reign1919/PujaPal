document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadPandals() {
    try {
      const res = await fetch('south_night.json');
      const data = await res.json();

      // ✅ filter for NIGHT / NIGHTLIFE only
      const night = data.filter(p =>
        (p["Best Viewing Time"] || "").toLowerCase().includes("night")
      );

      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          pos => {
            renderPandals(night, pos.coords.latitude, pos.coords.longitude);
          },
          () => renderPandals(night, null, null) // fallback
        );
      } else {
        renderPandals(night, null, null);
      }
    } catch (err) {
      container.innerHTML =
