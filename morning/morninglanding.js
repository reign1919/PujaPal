document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadPandals() {
    try {
      const res = await fetch('south_morning.json');
      const data = await res.json();

      const morning = data.filter(p =>
        (p["Best Viewing Time"] || "").toLowerCase().includes("morning")
      );
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          pos => {
            renderPandals(morning, pos.coords.latitude, pos.coords.longitude);
          },
          () => {
            renderPandals(morning, null, null); // fallback no location
          }
        );
      } else {
        renderPandals(morning, null, null);
      }
    } catch (err) {
      container.innerHTML =
