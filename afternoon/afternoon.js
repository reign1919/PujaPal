document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadPandals() {
    try {
      const res = await fetch('south_afternoon.json');
      const data = await res.json();

      // ✅ filter for AFTERNOON only
      const afternoon = data.filter(p =>
        (p["Best Viewing Time"] || "").toLowerCase().includes("afternoon")
      );

      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          pos => {
            renderPandals(afternoon, pos.coords.latitude, pos.coords.longitude);
          },
          () => renderPandals(afternoon, null, null) // fallback
        );
      } else {
        renderPandals(afternoon, null, null);
      }
    } catch (err) {
      container.innerHTML =
  ...
  function renderPandals(pandals, userLat, userLng) {
    ...
    pandals.forEach((p, idx) => {
      const name = p["Pandal Name"];
      const location = p["__EMPTY"] || "";
      const mapLink = p["Maps"] || "";

      // premium = has location, normal otherwise
      const tier = location ? "premium" : "normal";
      ...
      card.innerHTML = `
        <div class="card-inner">
          <div>
            ...
            ${location ? `<p class="pandal-location">${escapeHtml(location)}</p>` : ""}
